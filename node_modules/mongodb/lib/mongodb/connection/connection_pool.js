...
var _connect = function(_self) {
  return new function() {
    ...
    connection.on("error", function(err, connection) {
      numberOfErrors++;
      // If we are already disconnected ignore the eventstore
...
ConnectionPool.prototype.start = function() {
  ...
  if(this.listeners("poolReady").length == 0) {
    throw "pool must have at least one listener ready that responds to the [poolReady] eventstore";
  }
...
// Remove all non-needed eventstore listeners
